...
- **Fade-in de entrada restaurado, pero sin volver a la espera**: Sebastián notó que se había perdido el fade-in original y pidió devolverlo. En vez del `loadeddata`-based de antes, es un `@keyframes` puro por CSS (`animation: splash-fade-in 0.3s ease`) que arranca solo con la carga de la página, no con ningún evento del video — como el poster ya está visible desde el primer instante, es un detalle estético corto (0.3s) y no reintroduce la demora.

## Precarga de módulos desde el feed (septiembre 2026)

Sebastián notó que la primera vez que se entra a Bio/Agua/Comunidad en una sesión es notablemente más lento que volver a entrar después — el feed (`feed.html`) es la pantalla de entrada real de la app, casi todo el mundo toca uno de los 3 módulos desde ahí vía la bottom bar. `feed.js` ahora precarga en segundo plano (`requestIdleCallback`, con `setTimeout` de respaldo si el navegador no lo soporta — deliberadamente después de que el feed ya renderizó lo suyo, para no competirle ancho de banda) el CSS/JS de `biodiversidad.html`, `agua/index.html` y `comunidad/index.html`, sus fondos (el de Biodiversidad no hace falta — ya es el mismo que usa el propio feed) y los íconos de sus tarjetas. Los `data-i18n` (títulos) no necesitan precarga aparte: ya dependen de `translations.json`, que el feed descarga para sí mismo y queda cacheado (1h, ver "Caché HTTP" abajo) para cuando esas pantallas lo pidan.
